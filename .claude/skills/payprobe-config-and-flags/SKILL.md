...
| `PAYPROBE_RUN_FIXTURES` | `0` (OFF) | ADR-0012 phase 3: `POST /runs` accepts `fixtures: {before: [ids], after: [ids]}` (saved scenarios run once around the run; a failed before-fixture BLOCKs everything, after-fixtures run in a finally and never change the verdict; outcomes under `summary.fixtures`, sign-off report and provenance, never in gates). OFF ⇒ a request naming fixtures is refused with 400. Flip to `1` after a real-environment run (phase 4). | experimental | `packages/orchestrator/api/main.py:_RUN_FIXTURES`, `_resolve_run_fixtures` |
