...
| `PAYPROBE_RUN_FIXTURES` | `1` (ON) | ADR-0012: `POST /runs` accepts `fixtures: {before: [ids], after: [ids]}` (saved scenarios run once around the run; a failed before-fixture BLOCKs everything, after-fixtures run in a finally and never change the verdict; outcomes under `summary.fixtures`, sign-off report and provenance, never in gates). `0` ⇒ a request naming fixtures is refused with 400 (escape hatch). Flipped ON 2026-09-25 after a real-environment run against the live registry + compose PostgreSQL. | ops | `packages/orchestrator/api/main.py:_RUN_FIXTURES`, `_resolve_run_fixtures` |
| `PAYPROBE_ISO8583_FORMAT_ENCODING` | `1` (ON) | ADR-0011: a simulator bound to a Message Format also takes the format's wire `encoding` profile (ascii / binary / axis dict), a legacy `framing.encoding` is folded away, and a format whose encoding disagrees with the config's own is a 400. `0`/`false`/`no` = pre-ADR posture (only DE table + presence bind; disagreement logged). Flipped ON 2026-09-24 after a real-environment run — reversibility escape hatch, not an experiment. | ops | `packages/orchestrator/api/main.py:_ISO8583_FORMAT_ENCODING`, `_apply_format_encoding` |
