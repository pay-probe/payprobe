"""Caller auth gate for agent-hub.
...
The registry decides what agents may do; its own door is locked with the
same bearer the other services require (an auth-service user JWT, or the
static service token). Role checks on top of the gate (who may create,
edit, publish, pause) live in :func:`require_roles`.
...
def _verify_jwt(token: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED, f"invalid token: {type(exc).__name__}"
        ) from exc
...
def require_roles(request: Request, roles: list[str] | set[str]) -> dict[str, Any]:
    ...
    if have & set(roles):
        return claims
    raise HTTPException(
        status.HTTP_403_FORBIDDEN, f"requires one of roles: {', '.join(sorted(roles))}"
    )
