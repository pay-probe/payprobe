...
wide and carries the length in one of two encodings (``framing.length_encoding``):
...
chaos engine all go through these two functions so the two ends of a PayProbe
...
def decode_length(prefix: bytes, byte_order: str = "big", encoding: str = "binary") -> int:
    """The length a prefix declares; for ``ascii`` a non-digit prefix is a
    ``ValueError`` naming the bytes, the usual sign of a framing mismatch."""
    if encoding == "ascii":
