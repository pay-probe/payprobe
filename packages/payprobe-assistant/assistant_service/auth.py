"""Caller auth gate for the assistant service.
...
Until this gate existed the assistant only authenticated *to* its upstreams —
anyone who could reach :8400 could drive autonomous config writes with the
service's credentials. Now callers must present the same bearer the other
services require (an auth-service user JWT, or the static service token).
...
#: Liveness + API reference stay open; everything else (chat, revert) is gated.
