...
from payprobe_common import llm_provider as _llm
...
# Moved to payprobe_common.llm_provider (ADR-0010: agent-hub shares it).
# Names are kept so callers/tests keep their imports.
_openai_tools = _llm._openai_tools
_to_openai_messages = _llm._to_openai_messages
_parse_openai = _llm._parse_openai
_anthropic_tools = _llm._anthropic_tools
_to_anthropic_messages = _llm._to_anthropic_messages
_parse_anthropic = _llm._parse_anthropic


def provider_tool_call(convo: list[dict], tools: list[dict], llm: dict) -> CallerResult:
    return _llm.provider_tool_call(convo, tools, llm, rest.post_json)
