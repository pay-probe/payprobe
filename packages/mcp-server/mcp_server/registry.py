...
TOOL_GROUPS: list[tuple[str, list[tuple[str, str, dict]]]] = [
    ...
    ("Agents & workflows (ADR-0010)", [
        ("list_agents", "List agents", READ),
        ("get_agent", "Get agent", READ),
        ("wake_agent", "Wake agent (one heartbeat)", START),
        ("list_heartbeats", "List heartbeats", READ),
        ("get_heartbeat", "Get heartbeat", READ),
        ("cancel_heartbeat", "Cancel heartbeat", CANCEL),
        ("list_workflows", "List workflows", READ),
        ("get_workflow", "Get workflow", READ),
        ("run_workflow", "Run workflow", START),
        ("list_workflow_runs", "List workflow runs", READ),
        ("get_workflow_run", "Get workflow run", READ),
        ("cancel_workflow_run", "Cancel workflow run", CANCEL),
        ("list_approvals", "List approvals inbox", READ),
    ]),
    ("Go/No-Go sign-off", [
...
RESOURCE_SPECS: list[tuple[str, str, str, str]] = [
    ...
    ("payprobe://agents", "list_agents", "Agents",
     "Agent principals in the agent-hub registry (ADR-0010)."),
    ("payprobe://workflows", "list_workflows", "Workflows",
     "Workflow definitions (JSON DAGs with approval gates)."),
    ("payprobe://approvals", "list_approvals", "Approvals inbox",
     "Workflow runs waiting for a human decision."),
]
